...
bool personal_computer::is_valid_int(std::string s) {
  for (int i = 0; i < s.size(); i++) {
    if (!std::isdigit(s[i])) {
      std::cout << s[i] << " is not valid \n";
      return false;
    }
  }
  return true;
...
bool personal_computer::validate_storage_size(std::string init_storage_size) {
  std::string storage_sizes[5] = {"128gb", "256gb", "512gb", "1tb", "2tb"};
  for (int i = 0; i < 5; i++) {
...
std::string personal_computer::to_string(personal_computer computer) {
  std::string s = "Manufacturer: " + computer.manufacturer + '\n' +
                  ...
                  "Ram: " + std::to_string(computer.ram) + '\n' +
